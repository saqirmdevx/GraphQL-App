...
export const generateText = (minSyl = 2, maxSyl = 6, maxLen = 8, addSpaces = false) => {
  // generates a random name with a variable max length / syllable amount
  const sylAmount = Math.min(random(minSyl, maxSyl), random(minSyl, maxSyl))
  let n = ''

  for(let i = sylAmount; i > 0 && n.length < maxLen; --i) {
        const rnd = random(0, syllables.length - 1);
        if (addSpaces)
          n += " ";
        n += syllables[rnd];
  }
