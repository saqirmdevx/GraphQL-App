...
const generateText = (minSyl = 2, maxSyl = 6, maxLen = 8, addSpaces = false) => {
    // generates a random name with a variable max length / syllable amount
    const sylAmount = Math.min((0, misc_1.random)(minSyl, maxSyl), (0, misc_1.random)(minSyl, maxSyl));
    let n = '';
    for (let i = sylAmount; i > 0 && n.length < maxLen; --i) {
        const rnd = (0, misc_1.random)(0, syllables.length - 1);
        if (addSpaces)
            n += " ";
        n += syllables[rnd];
    }
